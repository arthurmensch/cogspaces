...
with open(join(dir_path, 'tasks.yml'), 'r') as f:
    TASKS = yaml.load(f, Loader=yaml.FullLoader)
STUDIES = pd.read_csv(join(dir_path, 'studies.csv'))
STUDIES = STUDIES.rename(columns={'Name': 'study', 'Description': 'description', 'Cite': 'reference'})
COMMENTS = {
    'hcp': 'The Human Connectome Project',
    'la5c': 'UCLA LA5C consortium',
    'archi': 'The ARCHI project',
...
def clean_targets(targets):
    ...
    targets = targets.merge(STUDIES, on='study')
    targets['comment'] = ''
    targets['reference'] = targets['reference'].map(lambda x: x.replace('citet', 'cite'))

    targets[['study', 'task', 'contrast']] = targets[['study', 'task', 'contrast']].applymap(
        lambda x: str(x).replace('_', ' '))
    targets['contrast'] = targets['contrast'].apply(lambda x: x.lower())


    def apply(target):
        study = target['study'].iloc[0]
        if study in TASKS:
            target['task'] = target['task'].replace({str(k): v for k, v in TASKS[study].items()})
            if study == 'brainomics':
                target['task'] = 'Localizer'
        elif target['task'].nunique() == 1:
            target['task'] = target['description']
        elif study == 'hcp':
            target['task'] = target['task'].map(lambda x: x.title())
        if study in COMMENTS:
            target['comment'] = COMMENTS[study]

        return target

    targets = targets.groupby(by='study').apply(apply)
    return targets